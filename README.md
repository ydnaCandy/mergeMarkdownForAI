...
## 背景
従来の生成AIによるドキュメント読み込みには、以下の課題が存在する。
- AI側の制約: 多くのAIツールはディレクトリ単位の読み込みに対応しておらず、単一ファイルでの提供が求められる。
- 管理上のギャップ: 人間は情報を整理するためにディレクトリ構造を用いるが、これをAIに渡す際には手動での結合やアップロードが必要になる。

本スクリプトにて、意味を持つディレクトリごとにMarkdownファイルを自動でマージし、一つのコンテキストとして集約する。これにより、管理上の利便性を損なうことなく、AIへの情報提供を効率化できる。
